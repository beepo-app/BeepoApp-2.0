...
AsyncSnapshot<List<xmtp.Conversation>> useConversationList() => _useLookupStream(
...
AsyncSnapshot<List<xmtp.DecodedMessage>> useMessages(String topic) => _useLookupStream(
...
AsyncSnapshot<xmtp.DecodedMessage?> useLastMessage(String topic) {
  ...
  return AsyncSnapshot.withData(messages.connectionState, messages.hasData && messages.data!.isNotEmpty ? messages.data!.first : null);
}
...
Future<void> Function() useMessagesRefresher(String topic) => () => session.refreshMessages([topic]);
...
AsyncSnapshot<T> _useLookupStream<T>(
  ...
  List<Object?> keys = const <Object>[],
    ...
    fh.useStream(fh.useMemoized(() => StreamGroup.mergeBroadcast([find().asStream(), watch()]), keys));
